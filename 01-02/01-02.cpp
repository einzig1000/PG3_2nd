...
int extractStudentNumber(const std::string& email)
{
	std::string numberPart;
	for (size_t i = 0; i < email.size(); ++i)
	{
		if (std::isdigit(email[i]))
		{
			numberPart += email[i];
		}
	}
...
bool compareByStudentNumber(const std::string& a, const std::string& b)
{
...
int main()
{
	...
	{
		std::cout << " ---------------------------------------------------------------------------------\n";
		std::for_each(std::begin(list), std::end(list), [](std::string& str) { std::cout << str << "\n"; });

		std::sort(list.begin(), list.end(), compareByStudentNumber);
